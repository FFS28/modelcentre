...
class ShopController extends Controller
{
    ...
    {
        $results = $this->velocityProductRepository->searchProductsFromCategory(
            request()->all()
        );
        ...
        if ($product) {
            ...
            $response = [
                ...
                'details' => [
                    ...
                    'totalReviews' => $productReviewHelper->getTotalReviews(
                        $product
                    ),
                    'rating' => ceil(
                        $productReviewHelper->getAverageRating($product)
                    ),
        ...
        switch ($slug) {
            ...
            case 'featured-products':
                ...
                if ($slug == 'new-products') {
                    $products = $this->velocityProductRepository->getNewProducts(
                        $count
                    );
                } elseif ($slug == 'featured-products') {
                    $products = $this->velocityProductRepository->getFeaturedProducts(
                        $count
                    );
                }
                    ...
                    'products' => $products
                        ->map(function ($product) {
                            if (
                                ...
                                return $this->velocityHelper->formatProduct(
                                    $product
                                );
                            } else {
                ...
                if ($categoryDetails) {
                    ...
                    foreach ($products as $product) {
                        ...
                        $productDetails = array_merge(
                            $productDetails,
                            $this->velocityHelper->formatProduct($product)
                        );
    ...
    public function fetchCategories()
    {
        ...
        return [
            'categories' => $formattedCategories[0]['children'],
        ];
    }
    ...
    public function fetchFancyCategoryDetails($slug)
    {
        ...
        if ($categoryDetails) {
            $response = [
                'status' => true,
                'categoryDetails' => $this->getCategoryFilteredData(
                    $categoryDetails
                ),
            ];
        }
    ...
    public function getItemsCount()
    {
        if (
            ...
            if (
                ...
                $wishlistItemsCount = $this->wishlistRepository
                    ->getModel()
                    ->leftJoin(
                        'products as ps',
                        'wishlist.product_id',
                    ...
                    ->where(function ($qb) {
                        $qb
                            ...
                            ->orwhereIn('ps.type', ['simple', 'virtual'])
                            ->where('pv.qty', '>', 0);
                    })
    ...
    public function getDetailedProducts()
    {
        if ($items = request()->get('items')) {
            ...
            $productCollection = $this->velocityHelper->fetchProductCollection(
                $items,
                $moveToCart
            );
     ...
     * @return array
     */
    public function getAllCustomizeOptions()
    {
        $data = [];
        ...
        foreach ($product_data as $key => $cur) {
            $product_subdata = DB::table('product_option_value')
                ->select('type_title', 'type_price')
                ->where('option_id', $cur->id)
                ->get();
            $data[$key] = [
                'cus_info' => ['title' => $cur->title, 'type' => $cur->type, 'price' => $cur->price],
                'cus_subinfo' => $product_subdata,
            ];
        }
